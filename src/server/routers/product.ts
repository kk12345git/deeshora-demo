...
export const productRouter = createTRPCRouter({
  ...
  smartSearch: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const descContains = words.map((w) => ({
        description: { contains: w, mode: "insensitive" as const },
      }));
      const tagsContains = words.map((w) => ({
      ...
      const relatedByDesc = await ctx.prisma.product.findMany({
        ...
        where: {
          ...
          OR:
            words.length > 0
              ? [{ AND: descContains }, { AND: tagsContains }]
              : undefined,
        },
        include: {
          category: { select: { name: true, slug: true } },
      ...
      await logActivity({
        ...
        message: `Search query: "${q}" (${exactByName.length + relatedByDesc.length} results)`,
        metadata: {
          query: q,
          results: exactByName.length + relatedByDesc.length,
        },
  ...
  seedCategories: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    const categories = [
      ...
      },
      {
        ...
        slug: "meat-fish",
        image:
          "https://images.unsplash.com/photo-1607623273573-7034ed8a9abd?w=800",
        sortOrder: 3,
      },
      {
        name: "Bakery & Dairy",
        slug: "bakery-dairy",
        image:
          "https://images.unsplash.com/photo-1550583724-125581f77833?w=800",
        sortOrder: 4,
      },
      {
        ...
        slug: "pharmacy",
        image:
          "https://images.unsplash.com/photo-1587854692152-cbe660dbbb88?w=800",
        sortOrder: 5,
      },
      {
      ...
      },
      {
        ...
        slug: "home-kitchen",
        image:
          "https://images.unsplash.com/photo-1556911220-e15224bbaf47?w=800",
        sortOrder: 8,
      },
      {
      ...
      },
      {
        ...
        slug: "toys-baby",
        image:
          "https://images.unsplash.com/photo-1532330393533-443990a51d10?w=800",
        sortOrder: 11,
      },
    ];
  ...
  autoAnalyze: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const detectedCategoryName = autoDetectCategory(
        input.name,
        input.description,
      );
      ...
      const category = await ctx.prisma.category.findFirst({
        where: {
          name: { contains: detectedCategoryName, mode: "insensitive" },
          isActive: true,
        },
      ...
      if (catLower.includes("vegetable") || catLower.includes("fruit")) {
        descriptionHints.push(
          'Mention freshness: "Farm-fresh", "Harvested today"',
        );
        descriptionHints.push(
          'Add origin: "Grown in Tamil Nadu", "Local farm"',
        );
        ...
        images: z
          .array(
            z
              ...
              .refine(
                (s) => s.length <= MAX_IMAGE_BASE64_BYTES,
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (product._count.orderItems > 0) {
        ...
        return {
          success: true,
          message:
            "Product has order history. It has been deactivated instead of deleted.",
        };
      }
  ...
  addReview: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existingReview = await ctx.prisma.review.findUnique({
        where: {
          productId_userId: { productId: input.productId, userId: ctx.user.id },
        },
      });
      if (existingReview) {
        throw new TRPCError({
