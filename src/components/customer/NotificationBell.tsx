...
import { Bell, Check, X, Loader2, Package, Percent, MessageCircle, AlertTriangle } from 'lucide-react';
...
export default function NotificationBell() {
  ...
  const { data: notifications = [], isLoading } = trpc.user.myNotifications.useQuery(undefined, {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'PROMOTION': return <Percent size={16} className="text-emerald-500" />;
      case 'SYSTEM': return <AlertTriangle size={16} className="text-orange-500" />;
    ...
    <div className="relative">
      <button
        ...
        className={`w-10 h-10 flex items-center justify-center rounded-2xl transition-all ${
          isOpen ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'
        }`}
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 flex h-4 w-4">
            ...
            <span className="relative inline-flex rounded-full h-4 w-4 bg-orange-600 text-white text-[9px] font-black items-center justify-center">
              {unreadCount}
            </span>
          </span>
        )}
          ...
          <>
            ...
            >
              <div className="px-5 py-4 border-b border-gray-50 dark:border-gray-800 flex items-center justify-between bg-gray-50/50 dark:bg-gray-900/50">
                <h3 className="font-black text-gray-900 dark:text-white uppercase tracking-widest text-xs">Notifications</h3>
                {unreadCount > 0 && (
                  <span className="text-[10px] font-black text-orange-500 uppercase tracking-tighter bg-orange-50 dark:bg-orange-950/30 px-2 py-0.5 rounded-full">
                    {unreadCount} New
                  </span>
                )}
              </div>
              ...
              <div className="max-h-[400px] overflow-y-auto divide-y divide-gray-50 dark:divide-gray-800">
                {isLoading ? (
                  ...
                  notifications.map((n) => (
                    <div 
                      key={n.id} 
                      className={`p-4 flex gap-3 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50 ${!n.isRead ? 'bg-orange-50/30 dark:bg-orange-950/10' : ''}`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-1">
                          <p className="font-bold text-xs text-gray-900 dark:text-white line-clamp-1">{n.title}</p>
                          {!n.isRead && (
                            <button 
              ...
              {notifications.length > 0 && (
                <Link 
                  ...
                  onClick={() => setIsOpen(false)}
                  className="block w-full py-3.5 text-center text-[10px] font-black text-gray-400 hover:text-orange-500 uppercase tracking-[0.2em] transition-colors border-t border-gray-50 dark:border-gray-800"
                >
                  View All Activity
                </Link>
              )}
