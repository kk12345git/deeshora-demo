...
export default function DeliveryMap({
  ...
  if (!isClient)
    return <div className="h-64 bg-gray-900 animate-pulse rounded-3xl" />;
    ...
    <div className="h-64 md:h-96 rounded-3xl overflow-hidden border border-gray-800 shadow-2xl relative">
      ...
      <div className="absolute bottom-4 right-4 z-10">
        <button
          onClick={() =>
            window.open(
              `https://www.google.com/maps/dir/?api=1&origin=${pickup[0]},${pickup[1]}&destination=${drop[0]},${drop[1]}&travelmode=driving`,
              "_blank",
            )
          }
        ...
        >
          <Navigation size={20} />
          <span className="text-sm font-black uppercase tracking-tight pr-2">
            Start Navigation
          </span>
        </button>
      </div>
