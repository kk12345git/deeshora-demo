...
export default function AdminDashboardLive({ initialStats, initialActivities }: AdminDashboardLiveProps) {
  const statsQuery = trpc.admin.stats.useQuery(undefined, {
    initialData: initialStats,
    refetchInterval: 30000, // Fallback polling every 30s
  });

  const activitiesQuery = trpc.admin.activities.useQuery({ limit: 5 }, {
  ...
  const maxRevenue = Math.max(...(stats?.monthlyRevenue?.map((r: any) => r.revenue) || [1]), 1);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 bg-orange-100 text-orange-600 text-[9px] font-black uppercase tracking-widest rounded-md animate-pulse">Live</span>
