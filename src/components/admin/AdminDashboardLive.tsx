...
export default function AdminDashboardLive({
  ...
  const activitiesQuery = trpc.admin.activities.useQuery(
    { limit: 5 },
    {
  ...
  useEffect(() => {
    ...
    const handleUpdate = () => {
      console.log("[AdminDashboard] Live update triggered");
      statsQuery.refetch();
  ...
  }, [statsQuery, activitiesQuery]);

  const maxRevenue = Math.max(
    ...(stats?.monthlyRevenue?.map((r: any) => r.revenue) || [1]),
    1,
  );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 bg-brand-100 text-brand-600 text-[9px] font-black uppercase tracking-widest rounded-md animate-pulse">
              Live
            </span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {[
          ...
          >
            ...
            >
              <div
                className={`w-12 h-12 bg-gradient-to-br ${card.gradient} text-white rounded-2xl flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform`}
              >
                {card.icon}
              </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-8 overflow-hidden relative group">
          ...
          <div className="flex items-center justify-between mb-10 relative z-10">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-500 flex items-center gap-2">
          ...
          <div className="flex items-end gap-5 h-56 relative z-10">
            {stats.monthlyRevenue.map((monthData: any, i: number) => {
              const pct = Math.max(
                (monthData.revenue / maxRevenue) * 100,
                monthData.revenue > 0 ? 8 : 2,
              );
              ...
              return (
                ...
                >
                  ...
                  <div className="w-full relative flex flex-col justify-end h-full">
                    <div
                      className={`w-full rounded-t-[1rem] transition-all duration-700 relative overflow-hidden animate-in slide-in-from-bottom-full ${
                        isLatest
                          ? "bg-gradient-to-t from-brand-600 to-brand-400 shadow-[0_0_20px_rgba(249,115,22,0.3)]"
                          : "bg-brand-100 group-hover/bar:bg-brand-200"
                      }`}
                      style={{
                        height: `${pct}%`,
                  ...
                  <span
                    className={`text-[10px] font-black uppercase tracking-widest ${isLatest ? "text-brand-500" : "text-gray-400"}`}
                  >
                    {new Date(monthData.month + "-02").toLocaleString(
                      "default",
                      { month: "short" },
                    )}
                  </span>
        ...
        <div className="space-y-6">
          <div className="bg-gray-950 rounded-[2.5rem] p-8 shadow-2xl shadow-gray-950/20 text-white relative overflow-hidden group">
            <div className="absolute -top-20 -right-20 w-64 h-64 bg-brand-600/10 rounded-full blur-[80px] group-hover:bg-brand-600/20 transition-all" />

            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-6 relative z-10">
              Quick Control
            </p>
                    ...
                    >
                      <span
                        className={
                          link.hot
                            ? "text-white"
                            : "text-gray-500 group-hover/btn:text-white transition-colors"
                        }
                      >
                        {link.icon}
                      </span>
                      <span className="text-sm font-black flex-1 uppercase tracking-wider">
                        {link.label}
                      </span>
                      {link.badge && (
                        <span className="text-[10px] font-black bg-white text-brand-600 px-2.5 py-0.5 rounded-lg shadow-sm">
                          {link.badge}
                        </span>
                      )}
                      <ArrowRight
                        size={14}
                        className="opacity-40 group-hover/btn:opacity-100 group-hover/btn:translate-x-1 transition-all"
                      />
                    </Link>
                  </div>
                ),
            ...
            <div className="space-y-5">
              {activities.length === 0 ? (
                ...
                activities.map((log: any) => (
                  <div
                    key={log.id}
                    className="flex gap-4 group/log items-start"
                  >
                    <div
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[11px] font-black text-gray-900 group-hover/log:text-brand-600 transition-colors line-clamp-1 uppercase tracking-tight">
                        {log.message}
                      </p>
              ...
              <Link
                href="/admin/activity"
                className="flex items-center justify-center w-full py-2 bg-gray-50 hover:bg-brand-50 text-gray-400 hover:text-brand-600 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
              >
                View Audit Trail
              </Link>
            </div>
