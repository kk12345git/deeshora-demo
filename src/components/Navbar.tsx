...
export default function Navbar() {
  ...
  const { data: desktopSuggestions, isFetching: isDesktopFetching } =
    trpc.product.suggest.useQuery(
      { query: debouncedDesktop },
      { enabled: debouncedDesktop.trim().length >= 2 },
    );
  const { data: mobileSuggestions, isFetching: isMobileFetching } =
    trpc.product.suggest.useQuery(
  ...
  return (
    <header className="sticky top-0 z-50 transition-all duration-300">
      {" "}
      <div
        className={`container mx-auto px-4 transition-all duration-500 ${scrolled ? "mt-2" : "mt-4"}`}
      >
        {" "}
        {/* ── Main Nav Row ───────────────────────────────────────────────── */}{" "}
        <motion.nav
          layout
          className={`flex items-center gap-2 md:gap-3 px-3 md:px-4 py-2 md:py-3 rounded-2xl md:rounded-[2rem] border transition-all duration-500 ${scrolled ? "bg-white/60 dark:bg-gray-900/60 backdrop-blur-3xl border-white/40 dark:border-white/10 shadow-[0_8px_32px_0_rgba(31,38,135,0.07)]" : "bg-white dark:bg-gray-900 border-transparent shadow-sm"}`}
        >
          {" "}
          {/* Logo */}{" "}
          <Link
          ...
          <div
            ref={desktopRef}
            className="hidden md:block flex-1 max-w-lg mx-2 relative"
          >
            {" "}
            <form onSubmit={handleDesktopSubmit}>
              ...
              >
                ...
                <button
                  type="submit"
                  className="m-1 flex items-center gap-1.5 bg-brand-500 hover:bg-brand-600 text-white font-black text-xs px-4 py-2 rounded-[0.875rem] transition-all shadow-md shadow-brand-500/20 flex-shrink-0"
                >
                  {" "}
                  <Search size={14} />{" "}
          ...
          <div className="flex items-center gap-1.5 ml-auto flex-shrink-0">
            ...
            {isSignedIn && user?.publicMetadata?.role === "ADMIN" && (
              <Link
                href="/admin"
                className="hidden md:flex items-center gap-1.5 text-xs font-black text-white bg-gray-900 hover:bg-gray-700 px-3 py-2 rounded-2xl transition-colors"
              >
                {" "}
                <LayoutDashboard size={13} /> Admin{" "}
              </Link>
            )}{" "}
            {/* Theme Toggle */} <ThemeToggle /> {/* Language Switcher */}{" "}
            <div className="hidden sm:block">
              {" "}
              <LanguageSwitcher />{" "}
            </div>{" "}
            {/* Notifications */} {isSignedIn && <NotificationBell />}{" "}
            {/* Mobile Search Button (shown on small screens) */}{" "}
            <button
              onClick={() => setMobileSearchOpen(!mobileSearchOpen)}
              className={`md:hidden w-10 h-10 flex items-center justify-center rounded-2xl transition-all ${mobileSearchOpen ? "bg-brand-500 text-white" : "hover:bg-gray-100 text-gray-600"}`}
              aria-label="Search"
            >
              {" "}
            ...
            <Link
              href="/cart"
              className="relative w-10 h-10 flex items-center justify-center rounded-2xl hover:bg-gray-100 transition-colors text-gray-700"
            >
              {" "}
              <ShoppingCart size={20} strokeWidth={2} />{" "}
              {cart.itemCount() > 0 && (
                <span className="absolute -top-1 -right-1 h-4 w-4 md:h-5 md:w-5 rounded-full bg-brand-600 text-white text-[9px] md:text-[10px] font-black flex items-center justify-center shadow-lg shadow-brand-500/40">
                  {" "}
                  {cart.itemCount()}{" "}
                </span>
              )}{" "}
            ...
            {isSignedIn ? (
              <div className="flex items-center gap-2">
                ...
                <Link
                  href="/profile"
                  className="hidden md:flex w-9 h-9 items-center justify-center rounded-xl hover:bg-gray-100 transition-colors text-gray-400 hover:text-brand-500"
                >
                  {" "}
                  <UserRound size={19} />{" "}
                </Link>{" "}
                <div className="hidden md:block">
                  ...
                  <UserButton
                    appearance={{
                      elements: {
                        userButtonAvatarBox:
                          "h-9 w-9 border-2 border-white shadow-md rounded-xl",
                      },
                    }}
                    afterSignOutUrl="/"
                  />{" "}
              ...
              <div className="flex items-center gap-2">
                ...
                <Link
                  href="/sign-in"
                  className="btn-primary py-2 px-5 rounded-2xl text-xs md:text-sm"
                >
                  {" "}
                  {t("sign_in")}{" "}
                </Link>{" "}
        ...
        <div
          className={`md:hidden transition-all duration-300 overflow-hidden ${mobileSearchOpen ? "max-h-48 opacity-100 mt-2" : "max-h-0 opacity-0"}`}
        >
          {" "}
          <div ref={mobileRef} className="relative">
            ...
            <form onSubmit={handleMobileSubmit}>
              ...
              >
                ...
                <button
                  type="submit"
                  className="m-1.5 flex items-center gap-1 bg-brand-500 hover:bg-brand-600 text-white font-black text-xs px-4 py-2.5 rounded-[0.875rem] transition-all shadow-md shadow-brand-500/20"
                >
                  {" "}
                  <Search size={14} />{" "}
                </button>{" "}
          ...
          <div className="fixed inset-0 z-[60]">
            ...
            >
              ...
              <div className="flex-grow overflow-y-auto p-6 space-y-2">
                ...
                {navLinks
                  .filter(
                    (l) => !l.role || user?.publicMetadata.role === l.role,
                  )
                  .map((link) => (
                    <Link
                      key={link.href}
                      href={link.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center gap-4 p-4 rounded-2xl hover:bg-brand-50 dark:hover:bg-brand-950/30 hover:text-brand-600 transition-all font-bold group"
                    >
                      {" "}
                      <div className="w-10 h-10 bg-gray-50 dark:bg-gray-900 group-hover:bg-brand-100 dark:group-hover:bg-brand-900/50 rounded-xl flex items-center justify-center transition-colors">
                        {" "}
                        <link.icon size={20} />{" "}
                      </div>{" "}
                      {link.name}{" "}
                ...
                <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-brand-50 dark:bg-brand-900/10">
                  {" "}
