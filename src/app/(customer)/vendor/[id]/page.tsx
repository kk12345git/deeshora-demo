...
import { 
  ...
  Info, ShieldCheck, Search, Filter
} from 'lucide-react';
...
export default function VendorPublicProfilePage() {
  ...
  const { data: vendor, isLoading, error } = trpc.vendor.byId.useQuery({ id });

  if (isLoading) return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="flex flex-col items-center gap-4">
        <Loader2 className="w-12 h-12 text-orange-500 animate-spin" />
        <p className="text-sm font-black text-gray-400 uppercase tracking-widest">Entering Studio...</p>
      </div>
    </div>
  );
  ...
  if (error || !vendor) return (
    <div className="min-h-screen flex items-center justify-center p-6 text-center">
      <div className="max-w-md space-y-4">
        ...
        <h1 className="text-2xl font-black text-gray-900 uppercase">Shop Not Found</h1>
        <p className="text-gray-500 font-medium">This store might have moved or is temporarily unavailable.</p>
        <button onClick={() => router.push('/')} className="btn-primary w-full">Back to Marketplace</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50/30 pb-20">
      ...
      <div className="relative h-64 md:h-80 lg:h-96 w-full overflow-hidden">
        {vendor.coverImage ? (
          <Image src={vendor.coverImage} alt="Cover" fill className="object-cover" priority />
        ) : (
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center md:items-end">
            ...
            <div className="flex-1 text-center md:text-left space-y-2">
               ...
               <p className="text-gray-500 font-medium text-lg lg:max-w-xl line-clamp-2">
                 {vendor.description || "One of Deeshora's most trusted fulfillment partners."}
               </p>
