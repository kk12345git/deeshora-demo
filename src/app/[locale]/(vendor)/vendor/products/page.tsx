      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 bg-white border-2 border-gray-100 hover:border-brand-200 text-gray-700 font-black text-sm px-5 py-3 rounded-2xl transition-all shadow-sm"
          >
            {" "}
            <Upload size={18} className="text-brand-500" /> Bulk Upload{" "}
          </button>{" "}
          <Link
            href="/vendor/products/new"
            className="inline-flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-black text-sm px-5 py-3 rounded-2xl transition-all shadow-lg shadow-brand-500/20 hover:-translate-y-0.5"
          >
            {" "}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 text-sm bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-brand-400 font-medium"
            />{" "}
            {search && (
              <button
                onClick={() => setSearch("")}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-3xl border border-gray-100">
          ...
          <p className="text-gray-400 text-sm mt-2 max-w-xs">
            Start by adding your first product so customers can order from your
            shop.
          </p>{" "}
          <Link href="/vendor/products/new" className="btn-primary mt-6">
            + Add Your First Product
          </Link>{" "}
        </div>
      ) : viewMode === "grid" ? (
        /* ── GRID VIEW ─────────────────────────────────────────────────────── */ <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {" "}
            ...
            >
              ...
              <div className="relative h-44 bg-gray-50">
                ...
                <div className="absolute top-3 left-3 flex gap-1.5">
                  ...
                  {product.isFeatured && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-brand-700 bg-brand-100 px-2.5 py-1 rounded-full">
                      {" "}
                ...
                {product.stock <= 5 && (
                  <div className="absolute top-3 right-3">
                    {" "}
                    <span
                      className={`inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full ${product.stock === 0 ? "text-red-700 bg-red-100" : "text-amber-700 bg-amber-100"}`}
                    >
                      {" "}
                      {product.stock === 0
                        ? "⚠ Out of stock"
              ...
              <div className="p-4 space-y-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-brand-500 font-bold uppercase tracking-wider mt-0.5">
                ...
                <div className="flex items-center gap-2 pt-1 border-t border-gray-50">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-xl transition-all flex-1 justify-center ${product.isActive ? "bg-emerald-50 text-emerald-700 hover:bg-emerald-100" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
                  >
                    {" "}
                    {product.isActive ? (
                      <ToggleRight size={14} />
                  ...
                  <Link
                    ...
                    className="w-9 h-9 bg-gray-100 hover:bg-brand-50 hover:text-brand-600 text-gray-500 rounded-xl flex items-center justify-center transition-all"
                  >
                    {" "}
                    <Edit3 size={15} />{" "}
                  </Link>{" "}
        ...
        /* ── BULK STOCK EDIT VIEW ──────────────────────────────────────────── */ <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          {" "}
          <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-3">
            ...
            <p className="text-xs font-bold text-gray-500">
              {" "}
              Edit stock quantities below. Press{" "}
              <kbd className="text-[10px] bg-gray-100 border border-gray-200 rounded px-1.5 py-0.5 font-mono">
                Save
              </kbd>{" "}
              per row to apply.{" "}
            </p>{" "}
          ...
          <div className="divide-y divide-gray-50">
            ...
            {filtered.map((product) => {
              ...
              const isDirty =
                ...
                pendingValue !== product.stock.toString();
              const isSaving = savingStock === product.id;
              return (
                <div
                  key={product.id}
                  className={`flex items-center gap-4 px-5 py-3.5 transition-colors ${isDirty ? "bg-brand-50/40" : "hover:bg-gray-50/50"}`}
                >
                  {" "}
                  {/* Thumbnail */}{" "}
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-20 text-center font-black text-sm border-2 rounded-xl py-2 outline-none transition-all ${isDirty ? "border-brand-400 bg-white text-gray-900" : "border-transparent bg-gray-100 text-gray-700"} ${product.stock <= 5 ? "text-amber-600" : ""}`}
                      />{" "}
                      <span className="absolute -bottom-4 left-0 right-0 text-center text-[9px] text-gray-400 font-bold">
                        {product.unit}
                      </span>{" "}
                    </div>{" "}
                    {isDirty ? (
                      <button
                        ...
                        disabled={isSaving}
                        className="flex items-center gap-1 text-xs font-black text-white bg-brand-500 hover:bg-brand-600 px-3 py-2 rounded-xl transition-all"
                      >
                        {" "}
                        {isSaving ? (
                          <Loader2 size={12} className="animate-spin" />
                      ...
                      <Link
                        href={`/vendor/products/${product.id}/edit`}
                        className="w-8 h-8 bg-gray-100 hover:bg-brand-50 hover:text-brand-500 text-gray-400 rounded-xl flex items-center justify-center transition-all"
                        title="Full edit"
                      >
                        {" "}
                        <Edit3 size={13} />{" "}
