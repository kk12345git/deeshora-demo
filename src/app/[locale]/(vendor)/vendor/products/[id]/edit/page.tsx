...
export default function EditProductPage() {
  ...
  const { data: product, isLoading: isLoadingProduct } =
    trpc.product.vendorProducts.useQuery({
      limit: 100,
    });
  ...
  useEffect(() => {
    if (currentProduct && !isInitialized) {
      setForm({
        ...
        gstRate: currentProduct.gstRate.toString(),
      });
      setImages(
        currentProduct.images.map((url) => ({ kind: "existing", url })),
      );
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-black text-gray-800 flex items-center gap-2">
              <Upload size={16} className="text-brand-500" /> Current Photos
            </h2>
          ...
          {images.length > 0 && (
            <div className="grid grid-cols-4 gap-3">
              {images.map((entry, i) => {
                const src =
                  entry.kind === "existing" ? entry.url : entry.preview;
                return (
                  <div
                    key={i}
                  ...
                  >
                    ...
                    {entry.kind === "existing" && (
                      <div className="absolute top-1 right-1 text-[10px] font-black bg-gray-800/70 text-white px-1.5 py-0.5 rounded-md">
                        Saved
                      </div>
                    )}
                    <button
              ...
              {images.length < 4 && (
                <button
                  ...
                  className="aspect-square rounded-xl border-2 border-dashed border-gray-200 hover:border-brand-300 hover:bg-brand-50/50 flex flex-col items-center justify-center text-gray-400 hover:text-brand-500 transition-all gap-1"
                >
                  <Plus size={20} />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          ...
          {discount > 0 && (
            <p className="text-xs text-emerald-600 font-black">
              ✅ Offering a {discount}% discount on MRP!
            </p>
          )}
        </div>
        {/* ── Inventory ─────────────────────────────────────────────────── */}
          ...
          <label className="flex items-center gap-3 cursor-pointer group">
            <div
              className={`w-11 h-6 rounded-full transition-all relative ${
                form.isFeatured ? "bg-brand-500" : "bg-gray-200"
              }`}
              onClick={() =>
            ...
            <Sparkles
              ...
              className={`ml-auto transition-colors ${
                form.isFeatured ? "text-brand-500" : "text-gray-300"
              }`}
            />
          </label>
        </div>
        ...
        <button
          ...
          className="w-full h-14 bg-gradient-to-r from-brand-500 to-brand-600 text-white font-black text-sm tracking-widest uppercase rounded-2xl shadow-xl shadow-brand-500/25 flex items-center justify-center gap-3 disabled:opacity-60 transition-all hover:-translate-y-0.5 hover:shadow-brand-500/40"
        >
