...
export default function NewProductPage() {
  ...
  const { data: analysis, isFetching: isAnalyzing } =
    trpc.product.autoAnalyze.useQuery(
  ...
  const applyAutoCategory = useCallback(() => {
    if (analysis?.suggestedCategoryId) {
      ...
      toast.success(`Category set to "${analysis.suggestedCategoryName}"`, {
        icon: "🤖",
      });
    }
  }, [analysis]);
  // Auto-apply if form.categoryId is empty and we have a suggestion  useEffect(() => {    if (!form.categoryId && analysis?.suggestedCategoryId && !autoFillApplied) {      setForm(p => ({ ...p, categoryId: analysis.suggestedCategoryId! }));    }  }, [analysis, form.categoryId, autoFillApplied]);
  // ─── Image handling ───────────────────────────────────────────────────────
  ...
  const isBlocked =
    vendorStatus?.status === "PENDING" || vendorStatus?.status === "SUSPENDED";
  if (isStatusLoading) {
    return (
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          onClick={() => router.back()}
          className="w-10 h-10 bg-white border border-gray-200 rounded-xl flex items-center justify-center text-gray-500 hover:bg-gray-50 transition-colors"
        >
          {" "}
          <ChevronLeft size={20} />{" "}
        </button>{" "}
        <div>
          ...
          <p className="text-gray-400 text-sm">
            Fill in the details to list your product on Deeshora
          </p>{" "}
        </div>{" "}
      </div>{" "}
        ...
        <div className="flex items-start gap-4 p-5 bg-amber-50 border border-amber-200 rounded-2xl">
          ...
          <div>
            ...
            <p className="text-sm text-amber-700 mt-1 leading-relaxed">
              {" "}
              Your vendor account is under review. You can fill in the product
              details now and save them, but products will only go live once an
        ...
        <div className="flex items-start gap-4 p-5 bg-red-50 border border-red-200 rounded-2xl">
          ...
          <div>
            ...
            <p className="text-sm text-red-600 mt-1">
              {" "}
              Your vendor account has been suspended. You cannot add new
              products at this time. Please contact{" "}
              <a
                href="mailto:Deeshorasupport@gmail.com"
      ...
      {vendorStatus?.status === "APPROVED" && (
        <div className="flex items-center gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-2xl">
          ...
          <p className="text-sm font-bold text-emerald-800">
            {" "}
            {vendorStatus.shopName} — Your account is approved. Products will go
            live immediately.{" "}
          </p>{" "}
        </div>
      )}{" "}
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          ...
          <h2 className="font-black text-gray-800 flex items-center gap-2">
            <Upload size={16} className="text-brand-500" /> Product Photos
          </h2>{" "}
          {/* Drop zone */}{" "}
          <div
            onDrop={handleDrop}
            ...
            className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all ${isDragging ? "border-brand-400 bg-brand-50" : "border-gray-200 hover:border-brand-300 hover:bg-brand-50/50"}`}
          >
            {" "}
            ...
            <div className="grid grid-cols-4 gap-3">
              ...
              {images.map((img, i) => (
                ...
                >
                  ...
                  />{" "}
                  {i === 0 && (
                    <div className="absolute top-1 left-1 text-[10px] font-black bg-brand-500 text-white px-1.5 py-0.5 rounded-md">
                      MAIN
                    </div>
                  )}{" "}
                  <button
              ...
              {images.length < 4 && (
                <button
                  type="button"
                  onClick={() => fileRef.current?.click()}
                  className="aspect-square rounded-xl border-2 border-dashed border-gray-200 hover:border-brand-300 flex items-center justify-center text-gray-400 hover:text-brand-500 transition-all"
                >
                  {" "}
                  <PlusIcon size={20} />{" "}
                </button>
          ...
          {errors.images && (
            <p className="text-xs text-red-500 font-bold">{errors.images}</p>
          )}{" "}
        </div>{" "}
        {/* ── Basic Info ──────────────────────────────────────────────── */}{" "}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          {" "}
          <h2 className="font-black text-gray-800 flex items-center gap-2">
            <Package size={16} className="text-brand-500" /> Basic Info
          </h2>{" "}
            ...
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${analysis.confidence === "high" ? "bg-emerald-200 text-emerald-800" : "bg-amber-100 text-amber-700"}`}
                  >
                    {" "}
                ...
                {analysis.suggestedCategoryId &&
                  form.categoryId !== analysis.suggestedCategoryId && (
                    <button
                      type="button"
          ...
          <FormField label="Category *" error={errors.categoryId}>
            ...
            <div className="relative">
              ...
              <select
                ...
                className={`input-style ${errors.categoryId ? "border-red-400 bg-red-50" : ""} ${analysis?.suggestedCategoryId === form.categoryId && form.categoryId ? "border-emerald-400 bg-emerald-50" : ""}`}
              >
                {" "}
                ...
                {categories?.map((c) => (
                  <option key={c.id} value={c.id}>
                    {" "}
                    {c.name}{" "}
                    {analysis?.suggestedCategoryId === c.id
                      ? "← AI Suggestion"
                      : ""}{" "}
                  </option>
                ))}{" "}
              </select>{" "}
              {analysis?.suggestedCategoryId === form.categoryId &&
                form.categoryId && (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          {" "}
          <h2 className="font-black text-gray-800 flex items-center gap-2">
            <IndianRupee size={16} className="text-brand-500" /> Pricing
          </h2>{" "}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          {" "}
          <h2 className="font-black text-gray-800 flex items-center gap-2">
            <Box size={16} className="text-brand-500" /> Inventory
          </h2>{" "}
          <div className="grid grid-cols-2 gap-4">
            {" "}
          ...
          <label className="flex items-center gap-3 cursor-pointer group">
            {" "}
            <div
              className={`w-11 h-6 rounded-full transition-all relative ${form.isFeatured ? "bg-brand-500" : "bg-gray-200"}`}
              onClick={() =>
            ...
            >
              {" "}
              <div
                className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${form.isFeatured ? "translate-x-5" : ""}`}
              />{" "}
            </div>{" "}
            <div>
              {" "}
              ...
              <p className="text-xs text-gray-400">
                Shown on the homepage featured section
              </p>{" "}
            </div>{" "}
            <Sparkles
              size={16}
              className={`ml-auto transition-colors ${form.isFeatured ? "text-brand-500" : "text-gray-300"}`}
            />{" "}
          </label>{" "}
        </div>{" "}
        {/* Submit */}{" "}
        <button
          type="submit"
          disabled={
            createMutation.isPending || vendorStatus?.status === "SUSPENDED"
          }
          className="w-full h-14 bg-gradient-to-r from-brand-500 to-brand-600 text-white font-black text-sm tracking-widest uppercase rounded-2xl shadow-xl shadow-brand-500/25 flex items-center justify-center gap-3 disabled:opacity-60 transition-all hover:-translate-y-0.5 hover:shadow-brand-500/40 disabled:hover:translate-y-0"
        >
          {" "}
        ...
        {vendorStatus?.status === "PENDING" && (
          <p className="text-center text-xs text-amber-600 font-bold">
            {" "}
            ⏳ Product will be saved but won&apos;t be visible to customers
