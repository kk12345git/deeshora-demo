...
export default function OrderDetailsPage({
  ...
  const {
    ...
    error,
    refetch,
  } = trpc.delivery.getOrderDetails.useQuery(
    {
    ...
    return (
      <div className="py-20 flex flex-col items-center justify-center text-center px-6">
        ...
        <p className="text-gray-500 mb-8">
          {error?.message ||
            "This order may have been cancelled or assigned to someone else."}
        </p>
        <Link href="/delivery" className="btn-primary px-8">
          Return to Dashboard
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={() => router.back()}
          className="w-10 h-10 bg-gray-900 border border-gray-800 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all"
        >
          <ArrowLeft size={20} />
        </button>
        <div>
      ...
      <DeliveryMap
        ...
        drop={[
          ...
          (order.address.coordinates as any)?.lng || 80.31,
        ]}
      ...
      >
        <div className="flex items-center gap-3">
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center ${isAssigned ? "text-blue-500" : "text-green-500"}`}
          >
            <Clock size={18} />
          </div>
          <span
            className={`text-xs font-black uppercase tracking-widest ${isAssigned ? "text-blue-500" : "text-green-500"}`}
          >
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 px-2">
          ...
          <h3 className="text-sm font-black text-gray-100 uppercase tracking-[0.1em]">
            Package Contents
          </h3>
        ...
        <div className="bg-gray-900/50 border border-gray-800 rounded-[2rem] overflow-hidden">
          {order.items.map((item, idx) => (
            <div
              key={idx}
              className={`p-4 flex items-center justify-between ${idx !== order.items.length - 1 ? "border-b border-gray-800/50" : ""}`}
            >
              <div className="flex items-center gap-4">
      ...
      {isAssigned && (
        <div className="bg-gray-900 border border-gray-800 rounded-[2rem] p-6 space-y-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-sm font-black text-gray-100 uppercase tracking-widest">
              Verify Delivery
            </h3>
          </div>

          <p className="text-xs text-gray-500 leading-relaxed">
            Please ask the customer to show their **WhatsApp Delivery Pass
            (QR)**. You can verify it here.
          </p>
          <div className="flex gap-2">
            <input
              type="text"
