...
const OrderMap = dynamic(() => import("@/components/customer/OrderMap"), {
  ssr: false,
  loading: () => (
    <div className="h-64 bg-gray-50 animate-pulse rounded-2xl flex items-center justify-center text-gray-400 font-bold">
      Loading Map...
    </div>
  ),
});
...
const STEPS: {
  ...
  },
  {
    ...
    label: "Preparing",
    emoji: "🍳",
    description: "Your items are being packed/prepared",
  },
  {
    status: "READY",
  ...
  if (isCancelled) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center gap-3 bg-red-50 rounded-2xl border border-red-100">
        <div className="w-16 h-16 bg-red-100 rounded-3xl flex items-center justify-center text-3xl">
          ❌
        </div>
        ...
        <p className="text-red-500 text-sm font-medium">
          This order was {status.toLowerCase()} and will not be delivered.
        </p>
      </div>
    );
  }
      ...
      <div className="relative pl-4 space-y-8">
        ...
        <div
          ...
          style={{
            height:
              currentIdx >= 0
                ? `calc(${(currentIdx / (STEPS.length - 1)) * 100}% - 32px)`
                : "0%",
          }}
        />
        ...
        {STEPS.map((step, i) => {
          ...
          const stepEvents = timelineEvents.filter(
            ...
            <div key={step.status} className="relative z-10 flex gap-6">
              ...
              <div className="flex-1 pb-2">
                <div className="flex justify-between items-start mb-1">
                  <p
                    className={`font-black text-base ${done ? "text-gray-900" : "text-gray-400"}`}
                  >
                    {step.label}
                  </p>
                  {latestEvent && (
                ...
                <p
                  className={`text-sm ${done ? "text-gray-600" : "text-gray-400"}`}
                >
                  {step.description}
                </p>
                ...
                {stepEvents.length > 0 && done && (
                  <div className="mt-3 space-y-2">
                    {stepEvents.map((e, idx) => (
                      <div
                        key={idx}
                        className="flex items-start gap-2 bg-gray-50/80 rounded-xl p-3 border border-gray-100/50"
                      >
                    ...
                    <div className="mt-4 bg-emerald-50 border border-emerald-100 rounded-2xl p-4 flex items-center justify-between">
                      ...
                      {deliveryPartner.phone && (
                        <a
                          ...
                          className="w-10 h-10 bg-white border border-emerald-200 text-emerald-600 rounded-xl flex items-center justify-center shadow-sm hover:scale-105 transition-transform"
                        >
    ...
    <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5 space-y-4 mt-3 animate-in slide-in-from-top-2 duration-300">
      <div>
        <p className="font-black text-gray-800 text-sm">
          Rate: <span className="text-brand-500">{productName}</span>
        </p>
        <p className="text-xs text-gray-400 mt-0.5">
          Your honest opinion helps future buyers
        </p>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={onDone}
          className="text-xs font-bold text-gray-400 hover:text-gray-600 px-3 py-2.5"
        >
          Cancel
        </button>
      </div>
...
export default function OrderTrackingPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = trpc.order.byId.useQuery({ id: orderId });
  const {
    updates,
    currentStatus: liveStatus,
    deliveryPartner: livePartner,
    isConnected,
      ...
      toast(
        (t) => (
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs">
              Redirecting to WhatsApp to share your location for faster
              delivery...
            </p>
            <button
              onClick={() => {
    ...
    return (
      <div className="container mx-auto px-4 py-24 text-center">
        ...
        <p className="font-black text-gray-900 text-xl">
          {error?.message ?? "We couldn't find that order."}
        </p>
        <button
          onClick={() => router.back()}
          className="mt-6 bg-gray-900 text-white px-6 py-3 rounded-xl font-bold hover:bg-gray-800"
        >
          ← Return Home
        </button>
      </div>
      ...
      <div className="container mx-auto max-w-2xl px-4 pt-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-center">
            ...
            <div
              className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-black uppercase mt-1 ${
                isConnected
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
          <div className="flex items-center justify-between pb-4 border-b border-gray-50 mb-4">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-brand-50 rounded-lg flex items-center justify-center text-brand-600 text-lg">
                🏪
              </div>
              <div>
              ...
              return (
                <div key={item.id}>
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="font-black text-gray-900 mt-1">
                        ₹{(item.total / item.quantity).toFixed(0)}
                      </p>
                    </div>
                  </div>
                  ...
                  {isDelivered && item.productId && !justReviewed && (
                    ...
                    >
                      <Star size={14} className="text-brand-500" /> Write a
                      Review
                    </button>
                  )}
                  {justReviewed && (
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm flex flex-col justify-between">
            ...
            <div
              className={`mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl w-fit text-[10px] font-black uppercase tracking-widest ${
                order.paymentStatus === "PAID"
