...
export default function CartPage() {
  ...
  const { data: availabilityData, isFetching: isCheckingAvailability } =
    trpc.product.checkCartAvailability.useQuery(
  ...
  useEffect(() => {
    if (config) {
      ...
      const threshold = config.find(
        (c: any) => c.key === "free_delivery_above",
      )?.value;
  ...
  const hasUnavailableItems = mergedItems.some(
    (i) => !i.isAvailable || i.isExceedingStock,
  );
  const cartTotal = total();
  const hasPhysicalItems = mergedItems.some((item) => item.type === "PHYSICAL");
  const isEligibleForFreeDelivery =
    cartTotal >= freeDeliveryThreshold || !hasPhysicalItems;
  ...
  if (items.length === 0) {
    return (
      <div className="container mx-auto px-4 py-24 text-center animate-in fade-in slide-in-from-bottom-8 duration-1000">
        ...
        <p className="mt-4 text-gray-500 font-medium max-w-xs mx-auto">
          Looks like you haven&apos;t discovered anything amazing yet.
        </p>
    ...
    <div className="container mx-auto px-4 py-12 max-w-6xl animate-in fade-in duration-700">
      <div className="flex items-center justify-between mb-10">
        ...
        <button
          ...
          className="text-xs font-black text-red-500 uppercase tracking-widest hover:bg-red-50 px-4 py-2 rounded-full transition-colors"
        >
          Clear all
        </button>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-12">
                ...
                <div className="w-24 h-24 rounded-3xl overflow-hidden flex-shrink-0 shadow-inner bg-gray-50 border border-gray-50 relative">
                  ...
                  {!item.isAvailable && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/40">
                      <span className="text-[10px] font-black uppercase text-white bg-red-500 px-2 py-1 rounded">
                        Unavailable
                      </span>
                    </div>
                  )}
                ...
                <div className="flex-grow text-center sm:text-left">
                  ...
                  {item.isExceedingStock && item.isAvailable && (
                    <p className="text-xs font-bold text-amber-600 mt-2 flex items-center justify-center sm:justify-start gap-1">
                      <AlertCircle size={12} /> Only {item.realStock} left in
                      stock
                    </p>
                  )}
                  {!item.isAvailable && (
        ...
        <div className="lg:col-span-5">
          <div className="bg-gray-950 rounded-[3rem] p-8 text-white sticky top-24 shadow-2xl shadow-brand-500/10 overflow-hidden group">
            ...
            <h2 className="text-2xl font-black italic mb-8 relative z-10 uppercase tracking-tight">
              Summary
            </h2>
            ...
            <div className="space-y-5 relative z-10">
              ...
              <div className="flex justify-between items-center text-gray-400 border-b border-white/5 pb-4">
                ...
                <span
                  className={`text-xs font-black uppercase tracking-wider ${isEligibleForFreeDelivery ? "text-green-400" : "text-white"}`}
                >
                  {!hasPhysicalItems
                    ? "Not Applicable"
            ...
            <div className="mt-10 relative z-10">
              {hasUnavailableItems ? (
                <button
                  disabled
                  className="btn-primary w-full py-5 text-base flex gap-3 opacity-50 cursor-not-allowed"
                >
                ...
                <Link
                  href="/checkout"
                  className="btn-primary w-full py-5 text-base flex gap-3 group/btn"
                >
              ...
              <button
                ...
                onClick={() => {
                  const businessNumber =
                    config?.find((c: any) => c.key === "business_whatsapp")
                      ?.value || "918939318865";
                  ...
                  window.open(
                    getWhatsAppUrl(businessNumber, message),
                    "_blank",
                  );
                }}
              ...
              >
                <MessageCircle size={18} fill="currentColor" /> Order via
                WhatsApp
              </button>
            </div>
