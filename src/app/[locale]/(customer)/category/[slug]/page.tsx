...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  else if (params.slug.includes("electronic"))
    ogImage = "/og/og-electronics.png";
  else if (params.slug.includes("gift") || params.slug.includes("fashion"))
    ogImage = "/og/og-gifts.png";
  ...
  return {
    ...
    keywords: [
      category.name,
      "Thiruvottriyur category",
      "Chennai local delivery",
      "Deeshora Chennai",
    ],
    openGraph: {
...
export default async function CategoryPage({ params }: Props) {
  ...
  return (
    <CategoryDetailsClient slug={params.slug} categoryName={category.name} />
  );
}
