...
export default function VendorProfileClient({
  ...
  const memberSince = new Date(vendor.createdAt).toLocaleDateString("en-IN", {
    month: "long",
    year: "numeric",
  });
  ...
  const storeSchema = {
    ...
    description:
      vendor.description ||
      `Verified local shop in ${vendor.city} on Deeshora.`,
    ...
    geo: {
      "@type": "GeoCoordinates",
      latitude: 13.161,
      // Approximate for Thiruvottriyur
      longitude: 80.3015,
    },
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center md:items-end">
            ...
            <div className="relative shrink-0">
              <div className="w-32 h-32 md:w-40 md:h-40 bg-white rounded-[2.5rem] p-1.5 shadow-xl -mt-20 md:-mt-24">
                <div className="w-full h-full bg-brand-500 rounded-[2rem] overflow-hidden flex items-center justify-center text-white text-5xl font-black relative">
                  {vendor.logo ? (
                    <Image
                      src={vendor.logo}
                      alt={vendor.shopName}
            ...
            <div className="flex-1 text-center md:text-left space-y-2">
              ...
              <p className="text-gray-500 font-medium text-lg lg:max-w-xl line-clamp-2">
                {vendor.description ||
                  "One of Deeshora's most trusted fulfillment partners in Thiruvottriyur."}
              </p>

              <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-2">
                <div className="flex items-center gap-1.5 text-xs font-bold text-gray-400 bg-gray-50 px-3 py-1.5 rounded-xl">
                  <MapPin size={14} className="text-brand-500" /> {vendor.city}
                </div>
                <div className="flex items-center gap-1.5 text-xs font-bold text-gray-400 bg-gray-50 px-3 py-1.5 rounded-xl">
                ...
                <div className="flex items-center gap-1.5 text-xs font-bold text-gray-400 bg-gray-50 px-3 py-1.5 rounded-xl">
                  <Calendar size={14} className="text-brand-500" /> Since{" "}
                  {memberSince}
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3 shrink-0">
              <a
                href={getWhatsAppUrl(
                  vendor.phone,
                  WHATSAPP_TEMPLATES.ENGLISH.GENERAL_INQUIRY(vendor.shopName),
                )}
                target="_blank"
        ...
        <div className="flex flex-col lg:flex-row gap-12">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div>
                ...
                <p className="text-gray-400 font-bold text-sm">
                  {filteredProducts.length} items currently in stock
                </p>
              </div>
              <div className="relative group min-w-[300px]">
                ...
                <input
                  ...
                  className="w-full h-14 pl-12 pr-4 bg-white border border-gray-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 font-bold transition-all shadow-sm"
                />
              </div>
            </div>
          ...
          <div className="lg:w-80 space-y-8">
            ...
            <div className="bg-brand-600 rounded-3xl p-8 text-white relative overflow-hidden group">
              ...
              <p className="text-brand-100 font-medium text-sm leading-relaxed mb-6 relative z-10">
                Every item from {vendor.shopName} undergoes strict quality
                checks before it arrives at your doorstep. We guarantee local
                hub freshness.
              </p>
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10 flex items-center gap-3 relative z-10">
                <ShieldCheck className="text-brand-100" />
                <p className="text-[10px] font-black uppercase tracking-[0.2em]">
                  Deeshora Certified Hub
                </p>
              </div>
            </div>
          </div>
