...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      <div
        className={`w-10 h-10 ${colorBg} ${colorText} rounded-xl flex items-center justify-center mb-3`}
      >
        {icon}
      </div>
      <p className="text-2xl font-black text-gray-900">{value}</p>
      <p
        className={`text-[10px] font-black uppercase tracking-widest ${colorText} mt-0.5`}
      >
        {title}
      </p>
...
export default function AdminAnalyticsPage() {
  ...
  const { data: platformData, isLoading: isPlatformLoading } =
    trpc.admin.platformAnalytics.useQuery({ period });
  const { data: vendorData, isLoading: isVendorLoading } =
    trpc.admin.vendorAnalytics.useQuery({ period });

  const maxRevenue = Math.max(
    ...(vendorData?.vendorStats.map((v) => v.revenue) ?? [1]),
    1,
  );
  const maxMonthlyRevenue = Math.max(
    ...(vendorData?.monthlyBreakdown.map((m: any) => Number(m.revenue)) ?? [1]),
    1,
  );
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm font-medium mt-1">
            Detailed analysis of marketplace growth and vendor performance.
          </p>
        </div>
        {/* Period Selector */}
        <div className="flex items-center gap-1.5 bg-white p-1.5 rounded-[1.5rem] border border-gray-100 shadow-sm">
          {PERIOD_OPTIONS.map((p) => (
            ...
            >
              <Calendar
                size={12}
                className={period === p ? "text-blue-400" : ""}
              />
              {PERIOD_LABELS[p]}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {Array(4)
            ...
            .map((_, i) => (
              <div
                key={i}
                className="h-32 bg-gray-100 rounded-[2rem] animate-pulse"
              />
            ))}
        </div>
      ) : (
      ...
      <div className="grid lg:grid-cols-3 gap-8">
              ...
              {vendorData?.monthlyBreakdown.map((m: any, i: number) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] font-black uppercase tracking-widest ${isLatest ? "text-blue-600" : "text-gray-400"}`}
                    >
                      {new Date(m.month + "-02").toLocaleString("default", {
                        month: "short",
                      })}
                    </span>
                  </div>
        ...
        <div className="bg-gray-950 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-gray-950/20 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-8 relative z-10">
            ...
            <Award
              size={16}
              className="text-amber-400 shadow-[0_0_10px_rgba(251,191,36,0.5)]"
            />
          </div>
          {isPlatformLoading ? (
            ...
            <div className="space-y-3 relative z-10">
              {platformData?.topVendors.map((v: any, i: number) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-xl font-black text-xs flex items-center justify-center flex-shrink-0 shadow-lg ${
                      i === 0
                        ? "bg-gradient-to-br from-amber-400 to-brand-500 text-white"
                        : i === 1
                          ? "bg-gradient-to-br from-gray-300 to-gray-500 text-white"
                          : i === 2
                            ? "bg-gradient-to-br from-brand-800 to-red-950 text-white"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-black truncate text-gray-100 group-hover/v:text-white transition-colors uppercase tracking-tight">
                      {v.shopName}
                    </p>
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">
                      {v._count?.orders ?? 0} Transactions
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-black text-blue-400 tracking-tighter">
                      ₹
                      {Math.round(Number(v._sum?.total ?? 0)).toLocaleString(
                        "en-IN",
                      )}
                    </p>
                  </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 font-medium">
                Comparative analysis of active marketplace vendors.
              </p>
            </div>
          </div>
          ...
          <div className="p-8 space-y-4">
            {Array(4)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className="h-16 bg-gray-50 rounded-[1.5rem] animate-pulse"
                />
              ))}
          </div>
        ) : (
          <div className="divide-y divide-gray-50">
            {vendorData?.vendorStats.map((vendor, i) => {
              ...
              const barPct = Math.max(
                (vendor.revenue / maxRevenue) * 100,
                vendor.revenue > 0 ? 3 : 0,
              );
              return (
                <div
                  key={vendor.vendorId}
                  className={`transition-all ${isExpanded ? "bg-blue-50/30" : "hover:bg-gray-50/50"}`}
                >
                  <button
                    onClick={() =>
                  ...
                  >
                    <span
                      className={`w-8 h-8 rounded-xl font-black text-xs flex items-center justify-center flex-shrink-0 transition-transform group-hover/row:scale-110 ${
                        i === 0
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="font-black text-sm text-gray-900 uppercase tracking-tight truncate">
                          {vendor.shopName}
                        </span>
                      </div>
                    ...
                    <div className="hidden xl:grid grid-cols-4 gap-8 flex-shrink-0 pr-8 border-r border-gray-100">
                      <div className="text-right min-w-[100px]">
                        <p className="font-black text-gray-900 text-base tracking-tighter">
                          ₹{Math.round(vendor.revenue).toLocaleString("en-IN")}
                        </p>
                      ...
                      <div className="text-right min-w-[100px]">
                        <p className="font-black text-emerald-600 text-base tracking-tighter">
                          ₹
                          {Math.round(vendor.vendorEarnings).toLocaleString(
                            "en-IN",
                          )}
                        </p>
                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">
                      ...
                      <div className="text-right min-w-[100px]">
                        <p className="font-black text-blue-600 text-base tracking-tighter">
                          ₹
                          {Math.round(vendor.commission).toLocaleString(
                            "en-IN",
                          )}
                        </p>
                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">
                    ...
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${isExpanded ? "bg-blue-600 text-white shadow-lg" : "bg-gray-50 text-gray-300"}`}
                    >
                      <ChevronDown
                        size={20}
                          ...
                          <div className="bg-white rounded-3xl border border-blue-100 p-6 shadow-xl shadow-blue-500/5">
                            ...
                            {vendor.topProducts.length === 0 ? (
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                                {vendor.topProducts.map(
                                  (p: any, pi: number) => (
                                    <div
                                      key={pi}
                                      className="flex items-center gap-4 bg-gray-50 border border-gray-100 rounded-[1.5rem] p-4 group/p hover:bg-white hover:border-blue-200 transition-all"
                                    >
