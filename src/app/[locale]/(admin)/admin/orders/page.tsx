        ...
        {TABS.map((tab) => (
          ...
          >
            ...
            {counts[tab] !== undefined && counts[tab]! > 0 && (
              <span
                className={`min-w-[1.25rem] h-5 flex items-center justify-center text-[9px] font-black px-1 rounded-md ${
                  activeTab === tab
      ...
      {isLoading ? (
        ...
        <div className="py-24 text-center bg-white rounded-[2.5rem] border-2 border-dashed border-gray-100 shadow-sm">
          ...
          <p className="text-gray-400 text-sm mt-2 max-w-xs mx-auto">
            Try adjusting your filters or wait for new incoming requests.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                >
                  ...
                  <div className="relative flex-shrink-0">
                    <div
                      className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl transition-all shadow-inner ${
                        order.status === "DELIVERED"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-black text-gray-900 text-lg tracking-tight uppercase">
                        #{order.id.slice(-6)}
                      </span>
                    ...
                    <div className="flex items-center gap-3 text-xs font-medium text-gray-400">
                      ...
                      <span className="truncate">{order.vendor?.shopName ?? "Platform Order"}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    {nextStatus && !isFinal && (
                      <button
                        ...
                        className="hidden md:flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white bg-brand-600 hover:bg-brand-700 px-4 py-2.5 rounded-xl transition-all shadow-lg shadow-brand-500/20 active:scale-95 disabled:opacity-40"
                      >
                    ...
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${isExpanded ? "bg-brand-500 text-white shadow-lg" : "bg-gray-50 text-gray-300 group-hover:text-gray-600 group-hover:bg-gray-100"}`}
                    >
                      <ChevronDown
                        size={20}
                  ...
                  <div className="px-6 pb-6 pt-2">
                    <div className="grid lg:grid-cols-3 gap-6 p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-white rounded-2xl p-4 border border-gray-100 space-y-3 shadow-sm">
                          <InfoRow
                            icon={<User size={12} />}
                            label="Client"
                            value={order.user.name ?? "Guest User"}
                          />
                          <InfoRow
                          ...
                          <div className="pt-2 border-t border-gray-50">
                            ...
                            <div className="flex gap-2">
                              ...
                              <p className="text-xs font-bold text-gray-700 leading-relaxed">
                                {order.vendor?.city ?? "Direct"} Marketplace Fulfillment
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl p-4 border border-gray-100 space-y-4 shadow-sm">
                          ...
                          <div className="pt-4 border-t border-gray-100 space-y-2">
                            <div className="flex justify-between items-center">
                              ...
                              <span
                                className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest ${order.paymentStatus === "PAID" ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"}`}
                              >
                                {order.paymentStatus}
                              </span>
                            </div>
                      ...
                      <div className="space-y-4">
                        ...
                        {isFinal ? (
                          <div
                            className={`p-5 rounded-2xl border-2 border-dashed text-center flex flex-col items-center gap-3 ${
                              order.status === "DELIVERED"
                          ...
                          >
                            <div
                              className={`w-10 h-10 rounded-full flex items-center justify-center ${order.status === "DELIVERED" ? "bg-emerald-100" : "bg-gray-100"}`}
                            >
                              <CheckCircle size={18} />
                            </div>
                            <div>
                              ...
                              <p className="text-[10px] font-medium opacity-70 mt-1">
                                Lifecycle completed for this transaction.
                              </p>
                            </div>
                          </div>
                        ) : (
                                ...
                                <button
                                  ...
                                  className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl border text-[10px] font-black uppercase tracking-widest transition-all ${
                                    isCurrent
                                      ...
                                      : "bg-white border-gray-100 text-gray-400 hover:border-brand-300 hover:text-brand-600 active:scale-[0.98] disabled:opacity-40"
                                  }`}
                                >
                                  <div className="flex items-center gap-3">
                                    ...
                                    updateStatus.variables?.orderId ===
                                      ...
                                      <div
                                        className={`w-1.5 h-1.5 rounded-full ${isCurrent ? "bg-brand-500 animate-pulse" : "bg-gray-200"}`}
                                      />
                                    )}
                                    {s.replace("_", " ")}
                                  </div>
...
function InfoRow({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 w-10 flex-shrink-0">
        {label}
      </span>
