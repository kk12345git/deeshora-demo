      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <button
            ...
            title="Refresh"
          >
            <RefreshCw size={18} className={isLoading ? 'animate-spin text-brand-500' : ''} />
          </button>
        </div>
      </div>
          ...
          >
            ...
            {counts[tab] !== undefined && counts[tab]! > 0 && (
              <span className={`min-w-[1.25rem] h-5 flex items-center justify-center text-[9px] font-black px-1 rounded-md ${
                activeTab === tab ? 'bg-white/20 text-white' : 'bg-brand-100 text-brand-600'
              }`}>
                {counts[tab]}
              </span>
            )}
                ...
                >
                  ...
                  <div className="relative flex-shrink-0">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl transition-all shadow-inner ${
                      ...
                      order.status === 'OUT_FOR_DELIVERY' ? 'bg-brand-50 text-brand-600' :
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${isExpanded ? 'bg-brand-500 text-white shadow-lg' : 'bg-gray-50 text-gray-300 group-hover:text-gray-600 group-hover:bg-gray-100'}`}>
                          ...
                          <div className="space-y-2.5 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                            {order.items.map((item, i) => (
                              <div key={i} className="flex items-center justify-between gap-4 py-1">
                                ...
                                <p className="text-xs font-black text-brand-600">₹{item.price}</p>
                              </div>
                            ))}
                          </div>
                      ...
                      <div className="space-y-4">
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                     {updateStatus.isPending && updateStatus.variables?.status === s && updateStatus.variables?.orderId === order.id
                                        ...
                                        : <div className={`w-1.5 h-1.5 rounded-full ${isCurrent ? 'bg-brand-500 animate-pulse' : 'bg-gray-200'}`} />
                                     }
                                     {s.replace('_', ' ')}
                                  </div>
                                  {isCurrent && <ChevronRight size={12} className="text-brand-500" />}
                                </button>
                              );
                            })}
                          </div>
                        ...
                        {order.paymentStatus === 'PENDING' && order.paymentMethod === 'MANUAL_UPI' && (
                          <div className="pt-4 border-t border-gray-100 mt-2 space-y-3">
                             <p className="text-[10px] font-black uppercase tracking-widest text-brand-500">Manual UPI Reconciliation</p>
