...
export default function CoveragePage() {
  ...
  const { data: areas = [], isLoading } =
    trpc.admin.getAllServiceAreas.useQuery();
  ...
  const createArea = trpc.admin.createServiceArea.useMutation({
    onSuccess: () => {
      utils.admin.getAllServiceAreas.invalidate();
      utils.admin.getServiceAreas.invalidate();
      setEditing(null);
    },
  });
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-2">
            {" "}
            <div className="w-2 h-2 rounded-full bg-brand-500 shadow-[0_0_10px_rgba(249,115,22,0.4)]" />{" "}
          ...
          <p className="text-gray-400 font-bold mt-3 text-sm flex items-center gap-2">
            {" "}
            <Globe size={14} className="text-emerald-500" /> Managing{" "}
            <span className="text-gray-900 font-black">
              geospatial availability
            </span>{" "}
            and delivery locality nodes.{" "}
          </p>{" "}
        </motion.div>{" "}
        <motion.button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        {[
          ...
          >
            {" "}
            <div
              className={`w-14 h-14 bg-${color}-50 rounded-2xl flex items-center justify-center text-${color}-600 group-hover:scale-110 transition-transform`}
            >
              {icon}
            </div>{" "}
            <div>
              ...
              <p
                className={`text-[8px] font-black text-${color}-500/50 uppercase tracking-[0.2em] mt-1`}
              >
                {desc}
              </p>{" "}
            </div>{" "}
      ...
      >
        ...
        <div className="flex items-center justify-between px-2">
          ...
          <h2 className="text-[10px] font-black text-gray-900 uppercase tracking-[0.4em] flex items-center gap-3">
            {" "}
            <Navigation size={16} className="text-brand-500" /> Global
            Positional Audit{" "}
          </h2>{" "}
          ...
          >
            ...
            <div className="bg-gray-950 rounded-[4rem] p-10 space-y-10 shadow-2xl relative">
              ...
              <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none">
                {" "}
                <ShieldCheck size={160} className="text-brand-500" />{" "}
              </div>{" "}
              <div className="flex items-center justify-between relative z-10">
                ...
                <div>
                  ...
                  <h2 className="text-2xl font-black text-white tracking-tight uppercase">
                    {" "}
                    {editing.isNew
                      ? "Initialize Locality Node"
                      : `Audit Terminal: ${editing.label}`}{" "}
                  </h2>{" "}
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mt-1">
                    Configuring Geospatial parameters
                  </p>{" "}
                </div>{" "}
                <button
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    ...
                    <Field label="Identity Label" dark>
                      ...
                      <input
                        ...
                        className="w-full h-16 px-6 bg-white/5 border-2 border-transparent rounded-2xl font-black text-white focus:border-brand-500 outline-none transition-all placeholder:text-gray-700 uppercase"
                    ...
                    <Field label="Zone Partition" dark>
                      ...
                      <input
                        value={editing.zone ?? ""}
                        onChange={(e) =>
                          setEditing((p) => ({ ...p, zone: e.target.value }))
                        }
                        className="w-full h-16 px-6 bg-white/5 border-2 border-transparent rounded-2xl font-black text-white focus:border-brand-500 outline-none transition-all placeholder:text-gray-700 uppercase"
                    ...
                    <Field label="Network Pincode" dark>
                      ...
                      <input
                        ...
                        className="w-full h-16 px-6 bg-white/5 border-2 border-transparent rounded-2xl font-black text-white focus:border-brand-500 outline-none transition-all placeholder:text-gray-700"
                        placeholder="600019"
                      />{" "}
                    </Field>{" "}
                  </div>{" "}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    ...
                    <Field label="Priority sequence" dark>
                      ...
                      <input
                        ...
                        className="w-full h-16 px-6 bg-white/5 border-2 border-transparent rounded-2xl font-black text-white focus:border-brand-500 outline-none transition-all"
                      />{" "}
                    </Field>{" "}
                      ...
                      <div className="flex gap-3 h-16">
                        ...
                        {[true, false].map((val) => (
                          <button
                            key={String(val)}
                            onClick={() =>
                              setEditing((p) => ({ ...p, isServiceable: val }))
                            }
                            className={`flex-1 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${editing.isServiceable === val ? (val ? "bg-emerald-500 text-white shadow-lg shadow-emerald-500/20" : "bg-brand-500 text-white shadow-lg shadow-brand-500/20") : "bg-white/5 text-gray-500 hover:bg-white/10"}`}
                          >
                            {" "}
                  ...
                  <div className="flex gap-4 pt-4">
                    ...
                    <button
                      ...
                      className="flex-1 h-20 bg-brand-600 hover:bg-brand-700 text-white rounded-[1.75rem] font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-all shadow-2xl active:scale-95 disabled:opacity-20"
                    >
                      {" "}
                      {isSaving ? (
                    ...
                    <button
                      onClick={() => setEditing(null)}
                      className="px-10 h-20 rounded-[1.75rem] bg-white/5 text-white font-black text-xs uppercase tracking-widest hover:bg-white/10 transition-colors"
                    >
                      {" "}
                      Abort{" "}
                    </button>{" "}
                  ...
                  <div className="flex items-center justify-between px-2">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="text-[10px] font-mono text-emerald-400/60">
                        {" "}
                        LAT:{" "}
                        {(editing.coordinates as any)?.lat?.toFixed(6) ||
                          "NULL"}{" "}
                      </div>{" "}
                      <div className="text-[10px] font-mono text-emerald-400/60">
                        {" "}
                        LNG:{" "}
                        {(editing.coordinates as any)?.lng?.toFixed(6) ||
                          "NULL"}{" "}
                      </div>{" "}
                    </div>{" "}
                  </div>{" "}
                  <div className="p-2 bg-white/5 rounded-[3rem] border border-white/5">
                    ...
                    <CoverageMap
                      center={
                        editing.coordinates
                          ? [
                              (editing.coordinates as any).lat,
                              (editing.coordinates as any).lng,
                            ]
                          : undefined
                      }
                      onLocationSelect={(lat, lng) =>
                        setEditing((p) => ({ ...p, coordinates: { lat, lng } }))
                      }
                    />{" "}
                  </div>{" "}
                  <div className="flex items-center gap-3 text-brand-500/40 px-6 py-4 bg-brand-500/5 rounded-2xl border border-brand-500/10">
                    {" "}
                    <AlertTriangle size={18} />{" "}
                    <p className="text-[9px] font-black uppercase tracking-[0.2em] leading-relaxed">
                      System warning: updating positional coordinates will
                      immediately affect vendor discovery radar for this
                      locality.
                    </p>{" "}
                  </div>{" "}
                </div>{" "}
              ...
              >
                ...
                <div className="flex items-center gap-4 px-10 py-8 border-b border-gray-50 bg-gray-50/30">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-0.5">
                      {zoneAreas.length} LOCALITIES INTEGRATED
                    </p>{" "}
                  </div>{" "}
                </div>{" "}
                {/* Table */}{" "}
                <div className="divide-y divide-gray-50">
                  ...
                  {zoneAreas.map((area) => (
                    <div
                      key={area.id}
                      className={`flex items-center gap-8 px-10 py-8 transition-all group ${!area.isActive ? "opacity-40 bg-gray-50/50" : "hover:bg-gray-50/80"}`}
                    >
                      {" "}
                      {/* Status pill */}{" "}
                        ...
                        <AnimatePresence mode="wait">
                          ...
                          {!area.isActive ? (
                            <motion.span
                              ...
                              initial={{ opacity: 0 }}
                              animate={{ opacity: 1 }}
                              className="inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-gray-400 bg-gray-100 px-3 py-1.5 rounded-full border border-gray-200"
                            >
                              {" "}
                              <EyeOff size={10} /> HIDE{" "}
                            </motion.span>
                            ...
                            <motion.span
                              ...
                              initial={{ opacity: 0 }}
                              animate={{ opacity: 1 }}
                              className="inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100"
                            >
                              {" "}
                            ...
                            <motion.span
                              ...
                              initial={{ opacity: 0 }}
                              animate={{ opacity: 1 }}
                              className="inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-brand-600 bg-brand-50 px-3 py-1.5 rounded-full border border-brand-100"
                            >
                              {" "}
                              <Clock size={10} /> SOON{" "}
                            </motion.span>
                      ...
                      <div className="flex-1 min-w-0">
                        {" "}
                        <p className="font-black text-gray-950 text-base uppercase tracking-tight group-hover:text-brand-600 transition-colors">
                          {area.label}
                        </p>{" "}
                        <div className="flex items-center gap-3 mt-1.5">
                          {" "}
                          <code className="text-[10px] font-mono font-black text-gray-300 bg-gray-50 px-2 py-0.5 rounded border border-gray-100 uppercase tracking-tighter">
                            IDX_{area.value}
                          </code>{" "}
                          <span className="w-1 h-1 rounded-full bg-gray-200" />{" "}
                          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                            {" "}
                            {area.pincode || "NO_ZIP"} · SEQ_
                            {area.sortOrder}{" "}
                          </p>{" "}
                        </div>{" "}
                      </div>{" "}
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                        ...
                        <button
                          ...
                          className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${area.isServiceable ? "bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white" : "bg-brand-50 text-brand-600 hover:bg-brand-600 hover:text-white"}`}
                        >
                          {" "}
                          {area.isServiceable ? (
                            <ToggleRight size={20} />
                        ...
                        <button
                          ...
                          className="w-12 h-12 rounded-2xl bg-gray-100 text-gray-400 hover:bg-brand-600 hover:text-white flex items-center justify-center transition-all"
                        >
                          {" "}
                          <Edit3 size={18} />{" "}
                        </button>{" "}
                      ...
                      <ChevronRight
                        size={18}
                        className="text-gray-100 group-hover:text-brand-500 transition-colors ml-4"
                      />{" "}
                    </div>
                  ))}{" "}
                </div>{" "}
        ...
        <div className="space-y-6 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <InfoItem
              ...
              desc="Indicates active fulfillment capability. Visible to all customers in selection matrices."
              color="text-emerald-400"
            />{" "}
            <InfoItem
              label="Latency Node"
              desc="Expansion target. Visible as 'Coming Soon' to build regional anticipation."
              color="text-brand-400"
            />{" "}
            <InfoItem
              label="Stealth Node"
              desc="Internal testing or deprecated zones. Completely invisible to end-users."
              color="text-gray-500"
            />{" "}
          </div>{" "}
        </div>{" "}
...
function Field({
  ...
  return (
    <div className="space-y-3">
      {" "}
      <label
        className={`text-[9px] font-black uppercase tracking-[0.3em] ${dark ? "text-gray-500" : "text-gray-400"}`}
      >
        {label}
      </label>{" "}
      {children}{" "}
