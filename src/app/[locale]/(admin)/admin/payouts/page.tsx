...
export default function AdminPayoutsPage() {
  const { data: vendorsData, refetch: refetchVendors } =
    trpc.admin.vendors.useQuery({});
  const {
    data: payoutHistory,
    isLoading: historyLoading,
    refetch: refetchHistory,
  } = trpc.admin.payouts.useQuery({});
  ...
  const pendingVendors = (vendorsData?.vendors ?? []).filter(
    (v) => v.pendingPayout > 0,
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedVendorId || !utrNumber || !amountInput) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm font-medium mt-1">
            Disburse earnings to{" "}
            <span className="text-gray-900 font-bold">
              {pendingVendors.length} active partners
            </span>{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <div className="w-12 h-12 bg-brand-50 rounded-2xl flex items-center justify-center text-brand-500 mb-4 group-hover:scale-110 transition-transform">
            <Clock size={24} />
          </div>
          ...
          <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-50">
            <span className="text-xs font-bold text-gray-400">
              {pendingVendors.length} shop
              {pendingVendors.length !== 1 ? "s" : ""} awaiting
            </span>
          </div>
        </motion.div>
        <motion.div
        ...
        >
          ...
          <p className="text-3xl font-black text-gray-900 tracking-tighter">
            ₹
            {(
              payoutHistory?.payouts.reduce((s, p) => s + p.amount, 0) ?? 0
        ...
        <div className="xl:col-span-4 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <span className="px-2 py-0.5 bg-brand-50 text-brand-600 text-[9px] font-black rounded-md uppercase tracking-widest">
              {pendingVendors.length} Action Items
            </span>
          </div>
          {pendingVendors.length === 0 ? (
            <div className="bg-white rounded-[2.5rem] border border-gray-100 p-10 text-center shadow-sm">
              ...
              <p className="text-gray-400 text-xs mt-2 font-medium">
                No pending settlements detected in the system.
              </p>
            </div>
          ) : (
                ...
                <motion.button
                  ...
                  className={`w-full group flex items-center gap-4 p-4 rounded-[1.75rem] border-2 transition-all duration-300 text-left ${
                    selectedVendorId === v.id
                      ? "border-brand-500 bg-white shadow-xl shadow-gray-200/50 scale-[1.02]"
                      : "border-white bg-white shadow-sm hover:border-brand-200"
                  }`}
                >
                  <div
                  ...
                  <div className="text-right">
                    <p className="font-black text-gray-900 text-sm">
                      ₹{v.pendingPayout.toLocaleString("en-IN")}
                    </p>
                    <p className="text-[9px] font-black text-brand-500 uppercase tracking-widest mt-0.5">
                      Due Now
                    </p>
                  </div>
                </motion.button>
          ...
          {selectedVendor &&
            (selectedVendor.bankAccount || selectedVendor.ifscCode) && (
              <motion.div
                ...
                <div className="space-y-4">
                  {selectedVendor.bankAccount && (
                    <div>
                      ...
                      <p className="text-lg font-black tracking-[0.2em] text-blue-400 font-mono">
                        {selectedVendor.bankAccount}
                      </p>
                    </div>
                  )}
                  {selectedVendor.ifscCode && (
                    <div>
                      ...
                      <p className="text-sm font-black tracking-widest font-mono text-gray-300">
                        {selectedVendor.ifscCode}
                      </p>
                    </div>
                  )}
        ...
        <div className="xl:col-span-8 space-y-8">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-8">
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="relative group">
                    ...
                    <select
                      ...
                      required
                      className="w-full h-16 pl-14 pr-12 text-sm font-black uppercase tracking-wider border-2 border-gray-50 bg-gray-50 rounded-2xl focus:outline-none focus:border-brand-400 focus:bg-white outline-none transition-all appearance-none"
                    >
                      <option value="">— Choose Vendor —</option>
                      {pendingVendors.map((v) => (
                        <option key={v.id} value={v.id}>
                          {v.shopName} (₹{v.pendingPayout.toLocaleString()})
                        </option>
                      ))}
                    </select>
                    <ChevronDown
                      size={16}
                      className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-300 pointer-events-none group-focus-within:rotate-180 transition-transform"
                    />
                  </div>
                </div>
                {/* Amount Field */}
                <div className="space-y-2.5">
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      required
                      placeholder="0.00"
                      className="w-full h-16 pl-14 pr-6 text-xl font-black tracking-tight border-2 border-gray-50 bg-gray-50 rounded-2xl focus:outline-none focus:border-brand-400 focus:bg-white outline-none transition-all"
                    />
                  </div>
                  {selectedVendor && (
                    <motion.p
                    ...
                    >
                      ...
                      <span className="text-gray-900">
                        ₹{selectedVendor.pendingPayout.toLocaleString()}
                      </span>
                    </motion.p>
                  )}
                </div>
              ...
              <div className="space-y-2.5">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 ml-1">
                  Universal Transaction Reference (UTR)
                </label>
                <div className="relative group">
                  <Hash
                    size={18}
                  ...
                  <input
                    ...
                    className="w-full h-16 pl-14 pr-6 text-sm font-black tracking-widest border-2 border-gray-50 bg-gray-50 rounded-2xl focus:outline-none focus:border-brand-400 focus:bg-white outline-none transition-all font-mono"
                  />
                </div>
              </div>
              <button
                type="submit"
                disabled={
                  processing || !selectedVendorId || !utrNumber || !amountInput
                }
                className="w-full h-16 bg-gray-950 text-white font-black text-xs uppercase tracking-[0.2em] rounded-[1.5rem] flex items-center justify-center gap-3 transition-all shadow-2xl shadow-gray-950/20 hover:bg-brand-600 disabled:opacity-20 active:scale-95"
              >
                {processing ? (
