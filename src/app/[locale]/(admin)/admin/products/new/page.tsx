      ...
      <form onSubmit={handleSubmit} className="card p-8 space-y-8 bg-white/80 backdrop-blur-md shadow-xl border-white/20">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
            {images.map((src, index) => (
              <div key={index} className="relative aspect-square group">
                ...
                {index === 0 && <div className="absolute bottom-2 left-2 badge bg-brand-500 text-white text-[10px] shadow-sm">MAIN VISUAL</div>}
              </div>
            ))}
            {images.length < 6 && (
        ...
        <section className="bg-brand-50 p-6 rounded-[2rem] border-2 border-brand-100">
          <h3 className="text-lg font-bold text-brand-950 mb-4 flex items-center gap-2 uppercase tracking-tighter">
            <span className="w-6 h-6 bg-brand-500 text-white rounded-full flex items-center justify-center text-xs">5</span>
            Custom Commission Override
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            <div>
              <label htmlFor="commissionRate" className="block text-xs font-black text-brand-800 uppercase tracking-widest mb-1">Commission Rate (%)</label>
              <div className="relative">
                <input 
                 ...
                 />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 font-black text-brand-300">%</span>
              </div>
            </div>
        ...
        <section className="bg-brand-50/50 p-4 rounded-2xl border border-brand-100">
           <div className="flex items-center">
            <input type="checkbox" name="isFeatured" id="isFeatured" className="h-4 w-4 text-brand-600 border-gray-300 rounded focus:ring-brand-500" />
