  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    createProductMutation.mutate({
      ...
      commissionRate: formData.get("commissionRate")
        ? parseFloat(formData.get("commissionRate") as string) / 100
        : undefined,
      images,
    });
  };
      ...
      >
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
            {images.map((src, index) => (
              <div key={index} className="relative aspect-square group">
                <Image
                  ...
                  alt="Product Preview"
                  fill
                  className="w-full h-full object-cover rounded-2xl border-2 border-white shadow-md transition-transform group-hover:scale-[1.02]"
                  unoptimized
                />
                <button
                  type="button"
                ...
                {index === 0 && (
                  <div className="absolute bottom-2 left-2 badge bg-brand-500 text-white text-[10px] shadow-sm">
                    MAIN VISUAL
                  </div>
                )}
              </div>
            ...
            {images.length < 6 && (
              <label className="cursor-pointer aspect-square flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl text-gray-400 hover:bg-brand-50/50 hover:border-brand-200 transition-all">
                <Upload size={24} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            <div>
              <label
                ...
                className="block text-xs font-black text-brand-800 uppercase tracking-widest mb-1"
              >
                Commission Rate (%)
              </label>
              <div className="relative">
                <input
                  ...
                  min="0"
                  max="100"
                  className="w-full h-14 pl-4 pr-12 bg-white border-2 border-brand-100 rounded-2xl focus:border-brand-500 focus:ring-0 outline-none font-black text-brand-900"
                  placeholder="Leave blank for auto"
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 font-black text-brand-300">
                  %
                </span>
              </div>
            </div>
            <div className="text-xs text-brand-700 font-bold leading-relaxed">
              <p>
                Leave blank to use the{" "}
                <span className="underline italic">Category Default</span> or{" "}
                <span className="underline italic">Vendor Default</span>.
              </p>
              <p className="mt-1 opacity-70 italic">
                Example: Enter 12.5 for 12.5% commission on this specific
                product.
              </p>
            </div>
          </div>
        ...
        <section className="bg-brand-50/50 p-4 rounded-2xl border border-brand-100">
          <div className="flex items-center">
            ...
            >
              Feature this product prominently on the hometown storefront
            </label>
          </div>
        </section>
