  ...
  if (!currentProduct && isInitialized) {
    return (
      <div className="p-12 text-center">
        ...
        <button onClick={() => router.push('/admin/products')} className="mt-6 text-brand-500 font-bold hover:underline uppercase tracking-widest text-xs">← Back to List</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-6">
        <button onClick={() => router.back()} className="w-12 h-12 bg-white border-2 border-gray-100 rounded-2xl flex items-center justify-center text-gray-400 hover:text-brand-500 hover:border-brand-200 transition-all">
          <ChevronLeft size={24} />
        </button>
        <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-brand-950 rounded-[2.5rem] border border-brand-800 shadow-xl p-8 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div className="space-y-1.5">
              <label className="text-xs font-black text-brand-300 uppercase tracking-widest ml-1">Product-Specific Rate (%)</label>
              <div className="relative">
                <input
                  type="number"
                  step="0.1"
                ...
                />
                <span className="absolute right-6 top-1/2 -translate-y-1/2 font-black text-brand-500 text-xl">%</span>
              </div>
            </div>
            <div className="space-y-2">
              <p className="text-brand-100 text-sm font-bold leading-relaxed">
                Platform currently takes <span className="text-brand-400 font-black underline">{((currentProduct?.commissionRate ?? currentProduct?.category?.commissionRate ?? currentProduct?.vendor?.commissionRate ?? 0) * 100).toFixed(1)}%</span> from this product.
              </p>
