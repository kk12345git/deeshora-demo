...
export default function AdminTaxPage() {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Deeshora_GST_Report_${period}_${new Date().toISOString().slice(0, 10)}.csv`,
    );
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
        ...
        >
          ...
          <p className="text-gray-400 font-bold mt-3 text-sm flex items-center gap-2">
            {" "}
            <ShieldCheck size={14} className="text-emerald-500" /> Regulatory
            reconciliation and GST automated auditing{" "}
          </p>{" "}
        </motion.div>{" "}
        <motion.div
        ...
        >
          ...
          <div className="flex bg-gray-50 p-1 rounded-2xl border border-gray-100">
            ...
            {(["MONTHLY", "QUARTERLY", "ANNUAL"] as const).map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${period === p ? "bg-gray-900 text-white shadow-lg" : "text-gray-400 hover:text-gray-900"}`}
              >
                {" "}
                {p}{" "}
              </button>
          ...
          <button
            ...
            className="flex items-center gap-3 bg-gray-950 text-white px-8 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-brand-600 transition-all shadow-2xl active:scale-95 disabled:opacity-20"
          >
            {" "}
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-40 space-y-6">
          ...
          <div className="relative">
            {" "}
            <div className="w-24 h-24 border-4 border-gray-50 border-t-brand-500 rounded-full animate-spin" />{" "}
            <BarChart3
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-gray-200"
              size={32}
            />{" "}
          </div>{" "}
          <p className="font-black text-gray-400 uppercase tracking-[0.3em] animate-pulse">
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            >
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-brand-500 mb-6">
                Aggregate Liability
              </p>{" "}
              <h3 className="text-6xl font-black flex items-start gap-1 tracking-tighter">
                {" "}
                <span className="text-2xl mt-2 opacity-30">₹</span>{" "}
                {data?.summary.totalGst.toLocaleString("en-IN", {
            ...
            >
              ...
              <h3 className="text-5xl font-black text-gray-900 tracking-tighter">
                {" "}
                {data?.summary.vendorsCount}{" "}
                <span className="text-xl text-gray-200 uppercase tracking-widest font-black ml-2">
                  Partners
                </span>{" "}
              </h3>{" "}
              <div className="mt-10 flex items-center justify-between">
            ...
            <div className="px-10 py-10 border-b border-gray-50 flex flex-col xl:flex-row xl:items-center justify-between gap-8">
              ...
              <div>
                ...
                <h2 className="text-2xl font-black text-gray-900 flex items-center gap-3 uppercase tracking-tight">
                  {" "}
                  Liability Ledger{" "}
                  <span className="text-[10px] font-black text-emerald-500 px-3 py-1 bg-emerald-50 rounded-full tracking-widest">
                    REAL-TIME SYNC
                  </span>{" "}
                </h2>{" "}
              ...
              <div className="relative group">
                ...
                <Search
                  size={20}
                  className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-brand-500 transition-colors"
                />{" "}
                <input
                  type="text"
                  value={searchTerm}
                  ...
                  className="pl-16 pr-8 py-5 bg-gray-50 border-2 border-transparent rounded-[2rem] w-full xl:w-[400px] font-black text-xs uppercase tracking-widest focus:border-brand-200 focus:bg-white transition-all outline-none"
                />{" "}
              </div>{" "}
            </div>{" "}
              ...
              <table className="w-full">
                ...
                <thead>
                  ...
                  <tr className="bg-gray-50/50">
                    {" "}
                    <th className="px-10 py-6 text-left text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">
                      Merchant Identity
                    </th>{" "}
                    <th className="px-8 py-6 text-left text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">
                      GST Registration
                    </th>{" "}
                    <th className="px-8 py-6 text-right text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">
                      Taxable amount
                    </th>{" "}
                    <th className="px-8 py-6 text-right text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">
                      GST collected
                    </th>{" "}
                    <th className="px-10 py-6 text-right text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">
                      Final Settlement
                    </th>{" "}
                  </tr>{" "}
                </thead>{" "}
                      ...
                      >
                        ...
                        <td className="px-10 py-8">
                          ...
                          <div className="flex items-center gap-4">
                            {" "}
                            <div className="w-12 h-12 bg-gray-100 rounded-2xl flex items-center justify-center font-black text-gray-400 group-hover:bg-brand-50 group-hover:text-brand-500 transition-all uppercase">
                              {" "}
                              {v.shopName[0]}{" "}
                            </div>{" "}
                            <p className="font-black text-gray-900 uppercase tracking-tight text-base">
                              {v.shopName}
                            </p>{" "}
                          </div>{" "}
                        </td>{" "}
                        <td className="px-8 py-8">
                          ...
                          <div className="flex items-center gap-2">
                            {" "}
                            <span
                              className={`px-4 py-2 rounded-xl font-mono text-[10px] font-black tracking-widest ${v.gstNumber === "No GSTIN" ? "bg-brand-50 text-brand-600 border border-brand-100" : "bg-blue-50 text-blue-600 border border-blue-100"}`}
                            >
                              {" "}
                              {v.gstNumber}{" "}
                            </span>{" "}
                        ...
                        <td className="px-8 py-8 text-right">
                          ...
                          <p className="font-black text-gray-900 text-base tracking-tighter">
                            ₹{v.gstAmount.toLocaleString("en-IN")}
                          </p>{" "}
                        </td>{" "}
                        <td className="px-10 py-8 text-right">
                          ...
                          <div className="flex items-center justify-end gap-3">
                            ...
                            <div className="text-right">
                              ...
                              <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mt-0.5">
                                Ready for Disbursal
                              </p>{" "}
                            </div>{" "}
                            <ChevronRight
                              size={16}
                              className="text-gray-200 group-hover:text-brand-500 transition-all translate-x-0 group-hover:translate-x-1"
                            />{" "}
                          </div>{" "}
                        </td>{" "}
                      </motion.tr>
                  ...
                  {!filteredVendors?.length && (
                    <tr>
                      ...
                      <td colSpan={5} className="px-10 py-32 text-center">
                        ...
                        <p className="font-black text-gray-400 uppercase tracking-[0.2em] mt-2">
                          No merchant data found for the current filter criteria
                        </p>{" "}
                      </td>{" "}
                    </tr>
                  )}{" "}
