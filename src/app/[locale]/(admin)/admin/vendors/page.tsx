...
function StatusBadge({ status }: { status: VendorStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${cfg.bg} ${cfg.text}`}
    >
...
export default function AdminVendorsPage() {
  ...
  const updateStatus = trpc.admin.updateVendorStatus.useMutation({
    onSuccess: (_, vars) => {
      toast.success(
        `Vendor ${vars.status === "APPROVED" ? "approved ✅" : vars.status === "SUSPENDED" ? "suspended ⛔" : "updated"}`,
      );
      refetch();
    },
  ...
  const counts = {
    ...
    SUSPENDED:
      data?.vendors.filter((v) => v.status === "SUSPENDED").length ?? 0,
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="relative w-full sm:w-80 group">
            ...
            <input
              ...
              className="w-full pl-11 pr-5 py-3.5 text-sm bg-white border border-gray-200 rounded-[1.25rem] focus:outline-none focus:border-brand-400 focus:ring-4 focus:ring-brand-500/5 font-bold transition-all shadow-sm"
            />
          </div>
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-3 bg-gray-950 text-white px-7 py-3.5 rounded-[1.25rem] font-black text-xs uppercase tracking-widest hover:bg-brand-600 transition-all shadow-xl shadow-gray-950/10 active:scale-95 group"
          >
            <Store
              size={16}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div
          className={`p-5 rounded-3xl border shadow-sm transition-all ${counts.PENDING > 0 ? "bg-amber-50 border-amber-100 shadow-amber-500/5" : "bg-white border-gray-100"}`}
        >
          <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">
            Awaiting Review
          </p>
          <div className="flex items-center gap-2">
            <p
              className={`text-2xl font-black ${counts.PENDING > 0 ? "text-amber-600" : "text-gray-900"}`}
            >
              {counts.PENDING}
            </p>
            {counts.PENDING > 0 && (
              <span className="px-2 py-0.5 bg-amber-500 text-white text-[9px] font-black rounded-md animate-pulse">
                Action Required
              </span>
            )}
          </div>
          ...
          {(["ALL", "PENDING", "APPROVED", "SUSPENDED"] as Tab[]).map((tab) => (
            ...
            >
              {tab === "PENDING" && counts.PENDING > 0 && (
                <span className="w-4 h-4 bg-amber-500 text-white rounded-md text-[9px] flex items-center justify-center font-black">
                  {counts.PENDING}
                </span>
              )}
              {tab === "ALL"
      ...
      {isLoading ? (
        ...
        >
          ...
          <p className="text-gray-400 text-sm mt-2 max-w-xs mx-auto">
            Try adjusting your search filters or add a new vendor to the
            platform.
          </p>
        </motion.div>
      ) : (
            ...
            return (
              <motion.div
                ...
                className={`group bg-white rounded-[2rem] border transition-all duration-300 overflow-hidden ${
                  isExpanded
                    ? "border-brand-200 ring-4 ring-brand-500/5 shadow-2xl shadow-brand-500/10"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-black text-gray-900 text-lg tracking-tight truncate">
                        {vendor.shopName}
                      </span>
                  ...
                  {/* Interaction */}
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${isExpanded ? "bg-brand-500 text-white shadow-lg shadow-brand-500/20" : "bg-gray-50 text-gray-300 group-hover:text-gray-600 group-hover:bg-gray-100"}`}
                  >
                    <ChevronDown
                      size={20}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          <Detail
                            ...
                            value={`₹${vendor.totalEarnings.toFixed(2)}`}
                            highlight
                          />
                          <Detail
                            icon={<IndianRupee size={13} />}
                          ...
                          <Detail
                            ...
                            highlight={vendor.subscriptionStatus === "ACTIVE"}
                          />
                          {vendor.ifscCode && (
                            <Detail
                      ...
                      <div className="space-y-3">
                        ...
                        {vendor.subscriptionStatus !== "ACTIVE" && (
                          <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 mb-3">
                            ...
                            <button
                              onClick={() =>
                                confirmPayment.mutate({ vendorId: vendor.id })
                              }
                          ...
                          <div className="space-y-3">
                            <div>
                              ...
                              <input
                                ...
                                value={commPct}
                                onChange={(e) =>
                                  setCommissionInputs((p) => ({
                                    ...p,
                                    [vendor.id]: e.target.value,
                                  }))
                                }
                                className="w-full px-3 py-2 text-sm font-black border-2 border-gray-200 rounded-xl focus:border-brand-400 outline-none transition-all"
                              />
                              <p className="text-[10px] text-gray-400 mt-1">
                                Platform keeps {commPct}% of each order
                              </p>
                            </div>
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  handleAction(vendor.id, "APPROVED")
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleAction(vendor.id, "SUSPENDED")
                                }
                                disabled={updateStatus.isPending}
                                className="flex-1 flex items-center justify-center gap-1.5 bg-red-50 hover:bg-red-600 text-white font-black text-xs px-3 py-2.5 rounded-xl transition-all"
                              >
                                <Ban size={13} /> Reject
                              </button>
                            </div>
                            ...
                            <div>
                              ...
                              <div className="flex gap-2">
                                <input
                                  ...
                                  className="flex-1 px-3 py-2 text-sm font-black border-2 border-gray-200 rounded-xl focus:border-brand-400 outline-none transition-all"
                                />
                                <button
                                  onClick={() =>
                                    handleAction(vendor.id, "APPROVED")
                                  ...
                                  disabled={updateStatus.isPending}
                                  className="px-4 bg-gray-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-brand-600 transition-colors"
                                >
                                  Save
                                </button>
                              </div>
...
function Detail({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 w-20 flex-shrink-0">
        {label}
      </span>
      <span
        className={`text-xs font-bold truncate ${highlight ? "text-emerald-600" : warn ? "text-amber-600" : "text-gray-700"}`}
      >
        {value}
      </span>
    </div>
